...
- **UniFi Integration:** Create VLANs (single or bulk), configure port profiles using `native_networkconf_id` (network config IDs, not raw VLAN numbers) with `tagged_vlan_mgmt: "block_all"` and `excluded_networkconf_ids` to create pure access ports. Minimal PUT payload for compatibility with both USW switches and UAP devices (U6 IW). Fallback to cmd/devmgr. Device lookup by MAC address (resilient to device re-adoption). Port enable/disable support. Auto-enables `switch_vlan_enabled`. Provisioning status banner with 60s auto-polling after port changes.
...
- **AP-to-WLAN Auto-Sync:** When an AP/hybrid device is assigned to a unit with a WLAN (`unifiWlanId`), a dedicated `managed-{wlanName}` AP group is created (or reused) on the controller. The AP's MAC is added to that group and the group is added to the WLAN's `ap_group_ids`. The `_Unassigned-APs` group stays empty — it's only used as a "not broadcasting" parking lot. On removal or unit deletion, the AP is removed from the managed group (unless another unit with the same WLAN still references it). All sync calls are awaited and followed by force-provision. Unit deletion cascades port assignment cleanup (`deletePortAssignmentsByUnit`).
