...
- **Edit Functionality:** Networks (name, DHCP settings), WiFi (full settings), and Devices (name) can be edited via pencil icon buttons in the Actions column. Only Web UI-managed items (isManaged=true) show edit buttons for Networks and WiFi; controller-synced items show lock icons. Edits sync to UniFi controller. WiFi edit dialog includes: SSID name, security mode (WPA Personal/Open), WPA mode (WPA2/WPA3), band (2.4/5/both), PMF mode, AP group assignment, enabled/guest/hide SSID toggles, password. Collapsible Advanced section: Fast Roaming (802.11r), BSS Transition (802.11v), U-APSD, Proxy ARP, L2 Isolation, Group Rekey interval, DTIM settings, minimum data rate controls.
...
- **Network Isolation:** "Isolate Network" checkbox (checked by default) on both single and bulk network creation. Creates two LAN_IN firewall rules on UniFi controller to block inter-VLAN communication (drop rules keyed by UniFi network ID, with dynamic rule_index). Rules are cleaned up on network deletion. DB stores `networkIsolation` boolean; green shield icon shown in networks table for isolated networks.
