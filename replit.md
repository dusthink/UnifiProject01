...
- **AP Groups Management:** New "AP Groups" tab between WiFi and Devices. Lists controller AP groups, create/edit/delete groups with AP assignment. WiFi creation (single and bulk) includes AP Group selector — "All APs" default or custom group selection. UniFi client supports both v2 API (`/v2/api/site/{siteId}/apgroups`) and legacy REST (`/rest/apgroup`) with automatic fallback.
