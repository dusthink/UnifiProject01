...
- **Edit Functionality:** Networks (name, DHCP settings), WiFi (SSID name, password, enabled, guest), and Devices (name) can be edited via pencil icon buttons in the Actions column. Only Web UI-managed items (isManaged=true) show edit buttons for Networks and WiFi; controller-synced items show lock icons. Edits sync to UniFi controller for networks and WiFi.
