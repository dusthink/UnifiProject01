...
- **AP-to-WLAN Auto-Sync:** When an AP/hybrid device is assigned to a unit with a WLAN (`unifiWlanId`), the AP's MAC is automatically added to the WLAN's AP group(s) on the controller. On removal or unit deletion, the AP is removed from those groups (unless another unit with the same WLAN still references it). Unit deletion awaits AP sync completion, then cascades port assignment cleanup (`deletePortAssignmentsByUnit`).
