...
- **UniFi Integration:** Create VLANs (single or bulk), configure port profiles (multi-select port VLAN assignment with fallback to cmd/devmgr for UDM Pro compatibility), manage WLANs, discover devices (all per-controller, per-site). Port enable/disable support. Auto-enables `switch_vlan_enabled` on devices when assigning VLANs to ports (required for in-wall APs like U6 IW).
