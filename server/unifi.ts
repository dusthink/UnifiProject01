...
export class UnifiClient {
  ...
  async createWlan(siteId: string, opts: {
    ...
    let pendingApGroupIds: string[] | null = null;
    ...
    if (opts.broadcastApMacs && opts.broadcastApMacs.length > 0) {
      ...
      if (groupId) {
        pendingApGroupIds = [groupId];
      }
    } else if (opts.apGroupIds && opts.apGroupIds.length > 0) {
      pendingApGroupIds = opts.apGroupIds;
    }
    ...
    if (pendingApGroupIds && result?.data?.[0]?._id) {
      try {
        await this.updateWlan(siteId, result.data[0]._id, { ap_group_ids: pendingApGroupIds });
      } catch (e: any) {
  ...
  async createPpskWlan(siteId: string, name: string, networkId: string, ppskKeys: Array<{ password: string; vlanId: number; networkConfId?: string; description: string }>, advancedOpts?: Record<string, any>): Promise<any> {
    ...
    if (advancedOpts?.broadcastApMacs && advancedOpts.broadcastApMacs.length > 0) {
      ...
      pendingApGroupIds = advancedOpts.apGroupIds;
    }
    if (defaults.wlangroupId) body.wlangroup_id = defaults.wlangroupId;
    if (advancedOpts) {
  ...
  private findDeviceByMac(devices: any[], deviceMac: string): any {
    const macLower = deviceMac.toLowerCase();
  ...
  async setPortProfile(siteId: string, deviceId: string, deviceMac: string, portIdx: number, nativeVlan: number): Promise<any> {
    const [allDevices, allNetworks] = await Promise.all([
      this.getDevices(siteId),
      this.getNetworks(siteId),
    ]);
    const device = this.findDeviceByMac(allDevices, deviceMac);
    if (!device) throw new Error("Device not found on controller");

    const network = allNetworks.find((n: any) => n.vlan === nativeVlan || n.vlan_tag === nativeVlan);
    ...
    return this.updateDevicePortOverrides(siteId, device._id, deviceMac, portOverrides);
  }
